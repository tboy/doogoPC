...
`<div class="select-tree-module">
    <div class="total-select">已选择 {{endArr.length}}个</div>
    <div class="select-tree-content">
      <div class="tree-unit" v-for="(item,idx) in endArr" :key="idx">
        {{item.name}}
...
Vue.component('select-tree', {
  ...
  methods:{
    ...
    addLabel () {
      ...
      while (firstColumnNotCheckedAll.length !== 0) {
        ...
        firstColumnNotCheckedAll.push(...notCheckedAll);
			...
			selectData.map(function(item){
				 that.getEnd(item,that.endArr);
			});
			let temp=[];
		  that.endArr.map(function(item){
				  let isEnd = true;
				  temp.map(function(item2){
      ...
      console.log('endArr', this.endArr);
				debugger;
			if(that.treeColumnArr[that.treeColumnArr.length-1].length==0){
				that.treeColumnArr.pop();
			
			}
    },
		getEnd(selItem,endArr){
			let that =this;
			let child = [];
			
			this.data.map(function(item) {
				  if(selItem.name == item.parent_name) {
